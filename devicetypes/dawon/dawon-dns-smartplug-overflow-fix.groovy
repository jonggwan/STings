...
metadata {
    definition (name: "DawonDNS SmartPlug overflow fix", namespace: "jonggwan", author: "Booung_j", ocfDeviceType: "oic.d.smartplug", mnmn: "DawonDNS", vid: "generic-switch-power-energy") {
        capability "Actuator"
...
def parse(String description) {
    ...
    if (event) {
        ...
        if (event.name== "power") {
			...
			event.value = (event.value - state.energyBias)/energyDiv
            event.unit = "kWh"
        }
        ...
        attrData.each {
            ...
            if (it.value && it.clusterInt == zigbee.SIMPLE_METERING_CLUSTER && it.attrInt == ATTRIBUTE_HISTORICAL_CONSUMPTION) {
				...
				map.value = currentEnergy/energyDiv
                map.unit = "kWh"
            }
